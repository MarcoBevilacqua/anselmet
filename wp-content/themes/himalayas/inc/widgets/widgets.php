...
function himalayas_widgets_init() {
   ...
   register_widget( "himalayas_featured_press_posts_widget" );
   register_widget( "himalayas_our_team_widget" );
   register_widget( "himalayas_contact_widget" );
   register_widget( "himalayas_category_widget" );
   register_widget( "himalayas_call_to_action_custom_widget" );
 ...
 */
class himalayas_category_widget extends WP_Widget {
   function __construct() {
   ...
   function form( $instance ) {
      ...
      $service_menu_id = esc_attr( $instance[ 'category_menu_id' ] );
      $title = esc_attr( $instance['title'] );
      $text = esc_textarea( $instance['text'] );
      $number = absint( $instance[ 'number' ] ); ?>

      <p><?php _e( 'Note: Enter the Category Section ID and use same for Menu item. Only used for One Page Menu.', 'himalayas' ); ?></p>

      <p>
         <label for="<?php echo $this->get_field_id('category_menu_id'); ?>"><?php _e( 'Categry Section ID:', 'himalayas' ); ?></label>
         <input id="<?php echo $this->get_field_id('service_menu_id'); ?>" name="<?php echo $this->get_field_name('category_menu_id'); ?>" type="text" value="<?php echo $service_menu_id; ?>" />
      </p>
      ...
      <p><?php _e( 'Note: Create the pages and select Category Template to display Category pages.', 'himalayas' ); ?></p>
   <?php }

   function update( $new_instance, $old_instance ) {
      $instance = $old_instance;
      $instance[ 'category_menu_id' ] = strip_tags( $new_instance[ 'category_menu_id' ] );
      $instance[ 'title' ] = strip_tags( $new_instance[ 'title' ] );
   ...
   function widget( $args, $instance ) {
      ...
      global $post;
      $service_menu_id = isset( $instance[ 'category_menu_id' ] ) ? $instance[ 'category_menu_id' ] : '';
      $title = apply_filters( 'widget_title', isset( $instance[ 'title' ] ) ? $instance[ 'title' ] : '');
      $text = apply_filters( 'widget_text', empty( $instance['text'] ) ? '' : $instance['text'], $instance );
      $number = empty( $instance[ 'number' ] ) ? 6 : $instance[ 'number' ];
      ...
      foreach ( $pages as $page ) {
         $page_id = $page->ID;
         $template_name = get_post_meta( $page_id, '_wp_page_template', true );
         if( $template_name == 'page-templates/template-category.php' ) {
            array_push( $page_array, $page_id );
         }
      }
 ...
 * Call to action widget.
 */
class himalayas_call_to_action_custom_widget extends WP_Widget {
   function __construct() {
      $widget_ops = array( 'classname' => 'widget_call_to_action_block', 'description' => __( 'Use this widget to show the call to action section.', 'himalayas' ) );
      $control_ops = array( 'width' => 200, 'height' =>250 );
      parent::__construct( false, $name = __( 'TG: Call To Action (custom) Widget', 'himalayas' ), $widget_ops, $control_ops);
   }

   function form( $instance ) {
      $defaults[ 'background_color' ] = '#e44945';
      $defaults[ 'background_image' ] = '';
      $defaults[ 'text_main' ] = '';
      $defaults[ 'text_additional' ] = '';
      $defaults[ 'button_text' ] = '';
      ...
      <div class="<?php echo $bg_image_class . ' ' . $select; ?> clearfix" style="<?php echo $bg_image_style; ?>">
         ...
         <div class="custom-cta-bar">
            <div class="tg-container">
               ...
               <?php if( !empty( $button_text ) ) { ?>
                  <a class="cta-text-btn cta-custom" href="<?php echo $button_url; ?>" title="<?php echo esc_attr( $button_text ); ?>"><?php echo esc_html( $button_text ); ?></a>
               <?php } ?>
            </div>
         </div>
      </div>
                        ...
                        <div class="portfolio-hover">
                           ...
                           <?php /*
                              <a class="image-popup" href="<?php echo $image_popup_url; ?>" ><i class="fa fa-search-plus"></i></a>
                                */ ?>
                              <a href="<?php echo get_field('link'); ?>" title="<?php the_title_attribute();?>">
            ...
            <div class="tg-container">
               ...
               <div class="tg-column-wrapper clearfix">
                  ...
                  ?>
                     ...
                     <div class="tg-column-2 tg-column-bottom-margin">
                     <div class="blog-block">
                        ...
                        <?php if( has_post_thumbnail() ) { ?>
                           <!-- <div class="blog-img"> -->
                              <?php the_post_thumbnail('himalayas-news-custom', array('style' => 'float:left;padding:15px;')); ?>
                           <!-- </div> -->
                         <?php } ?>
                           <b class="blog-title">
...
/**
 * Featured Press Posts widget
 */
class himalayas_featured_press_posts_widget extends WP_Widget {

   function __construct() {
      $widget_ops = array( 'classname' => 'widget_featured_press_posts_block', 'description' => __( 'Display latest posts or posts of specific category', 'himalayas') );
      $control_ops = array( 'width' => 200, 'height' =>250 );
      parent::__construct( false,$name= __( 'TG: Press Posts', 'himalayas' ),$widget_ops);
   }

   function form( $instance ) {
      $defaults[ 'featured_press_menu_id' ] = '';
      $defaults['background_color'] = '#f1f1f1';
      $defaults['background_image' ] = '';
      $defaults[ 'title' ] = '';
      $defaults[ 'text' ] = '';
      ...
      $featured_menu_id = esc_attr( $instance[ 'featured_press_menu_id' ] );
      $background_color = esc_attr( $instance[ 'background_color' ] );
      $background_image = esc_url_raw( $instance[ 'background_image' ] );
      $title = esc_attr( $instance[ 'title' ] );
      $text = esc_textarea( $instance[ 'text' ] );
      ...
      <p><?php _e( 'Note: Enter the Featured Press Post Section ID and use same for Menu item. Only used for One Page Menu.', 'himalayas' ); ?></p>
       <p>
         <label for="<?php echo $this->get_field_id( 'featured_press_menu_id' ); ?>"><?php _e( 'Featured Press Post Section ID:', 'himalayas' ); ?></label>
         <input id="<?php echo $this->get_field_id( 'featured_press_menu_id' ); ?>" name="<?php echo $this->get_field_name( 'featured_press_menu_id' ); ?>" type="text" value="<?php echo $featured_menu_id; ?>" />
      </p>
   ...
   function update( $new_instance, $old_instance ) {
      ...
      $instance[ 'featured_press_menu_id' ] = strip_tags( $new_instance[ 'featured_press_menu_id' ] );
      $instance[ 'background_color' ] = $new_instance[ 'background_color' ];
      $instance[ 'background_image' ] = esc_url_raw( $new_instance[ 'background_image' ] );
      $instance[ 'title' ] = strip_tags( $new_instance[ 'title' ] );
      if ( current_user_can( 'unfiltered_html' ) )
   ...
   function widget( $args, $instance ) {
      ...
      global $post;
      $featured_menu_id = isset( $instance[ 'featured_press_menu_id' ] ) ? $instance[ 'featured_press_menu_id' ] : '';
      $background_color = isset( $instance[ 'background_color' ] ) ? $instance[ 'background_color' ] : '';
      $background_image = isset( $instance[ 'background_image' ] ) ? $instance[ 'background_image' ] : '';
      $title = apply_filters( 'widget_title', isset( $instance[ 'title' ] ) ? $instance[ 'title' ] : '');
      $text = isset( $instance[ 'text' ] ) ? $instance[ 'text' ] : '';
      ...
      <div <?php echo $section_id; ?> class="<?php echo $bg_image_class ?>" style="<?php echo $bg_image_style; ?>">
         ...
         <div class="section-wrapper">
            ...
            <div class="tg-column-2">
               <h2 style="text-transform: uppercase;">web</h2>
               <?php
                $web_posts = new WP_Query('category_name=dicono_di_noi&category_name=web&posts_per_page=1');
                //echo "web posts: " . $web_posts->post_count;
                while($web_posts->have_posts()) : $web_posts->the_post(); ?>
                <div class="tg-column-bottom-margin tg-column-center">
                     <div class="blog-block">
                        <?php if( has_post_thumbnail() ) { ?>
                           <div class="blog-img">
                      ...
                      <p class="blog-title press-featured">
                      <?php /*
                      <a href="<?php the_permalink(); ?>"
                      title="<?php the_title_attribute();?>">
                      <?php the_title(); ?>
                      </a> */?>
                      <a href="<?php echo get_field('testata'); ?>"
                      title="<?php the_title_attribute();?>">
                      <?php the_title(); ?>
                      </a>
                      </p>
            ...
            <div style="text-align: right;">
               <h2 style="text-transform:uppercase;">Stampa</h2>
            </div>
            <div>
               <?php
                $print_posts = new WP_Query('category_name=dicono_di_noi&category_name=stampa&posts_per_page=1');
                while($print_posts->have_posts()) : $print_posts->the_post(); ?>
